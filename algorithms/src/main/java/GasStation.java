...
public class GasStation {
    ...
    private static int canCompleteCircuit(int[] gas, int[] cost) {
        // Use max-sub-array approach by looping over circle (iterate twice).
        // If max sub-array exist and it consists of all elements of the array
        // then it's starting index is the answer
        int s = 0;
        int startIdx = -1;
        for (int i = 0; i < gas.length * 2; i++) {
            int idx = i % gas.length;
            int p = gas[idx] - cost[idx];

            if (startIdx == -1) startIdx = idx;

            s += p;

            // System.out.println("idx = " + idx + ", s = " + s + ", startIdx = " + startIdx);
            if (s < 0) {
                s = 0;
                startIdx = -1;
            } else if (i - startIdx == gas.length - 1) {
                return startIdx;
            }
        }
